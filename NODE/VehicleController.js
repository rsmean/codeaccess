...
class VehicleController  {
     ...
     getVehicles (req, res) {
          ...
          if(req.query.vehicleType && req.query.vehicleType != 'All') {
               condArr['specifications.vehicleType'] = new RegExp('^'+req.query.vehicleType+'$', "i");
          }
     ...
     myCars (req, res) {
          ...
          async.parallel({
                    ...
                    VehicleModel.aggregate([
                         ...
                         if(err) {
                              ...
                              ReviewModel
                                        ...
                                        .populate( {"path" : "userId", 'select' : 'name profile.path profile.photo'})
               ...
               myCars : function(callback){
                    ...
                    VehicleModel.aggregate(aggregationArr).exec(
                         function(err,resData) {
                              if(err) return res.json({resStatus:'error', msg :AppMessages.SERVER_ERR});
                              UserModel.populate(resData, {path:'userId', select: 'name profile.path profile.photo'},function(err,populatedResData){
                                   if (err) return res.json({resStatus:'error', msg : AppMessages.SERVER_ERR});
                                   if(err) {
                                        callback(true, null);
                                   } else {
               ...
               user : function(callback){
                    UserModel.findOne(
                         {_id : uId},{verification : 1, googleId : 1, facebookId : 1, name : 1, createdDate : 1, email : 1, profile : 1, favourites : 1})
                         .populate({
                              path: 'favourites',
                              select : 'name profile.path profile.photo specifications gallery ridingCost'
                         })
          ...
          view (req, res) {
               ...
               async.parallel({
                    ...
                    favourite : function (callback) {
                         if(req.query.uId) {
                              UserModel.findOne({isDeleted : false, _id : req.query.uId,  favourites : { $in : [vehicleId] }},function(err, resData){
                                   if(err) {
                                        callback(true, null);
                                   }
                                   if (resData) {
          ...
          async.waterfall([
               function (callback) {
                    ...
                    if(req.body.isMobile == 'true') {
                         ...
                         specifications.vehicleType = req.body.vehicleType;
     ...
     update (req, res) {
          ...
          if(req.body.isMobile == 'true') {
               ...
               if(req.body.details_country) {
                    //details.state = req.body.details_state;
                    updateObj["details.country"] = req.body.details_country;
               }
...
randomCars (req, res) {
     VehicleModel
          ...
          .exec (
               function (err, cars) {
                    if(err) return res.json({resStatus:'error', msg : AppMessages.SERVER_ERR});
                    return res.json({resStatus:'success', msg :'Popular Cars list', result : cars});
               }
          )
               ...
               fs.access(uploadDir, fs.F_OK, function(err) {
                    if (!err) {
                         ...
                         fs.readFile(files.file.path, function (err, data) {
                              fs.writeFile(uploadDir + newFileName, data, function (err) {
                                   if(err) return res.json({resStatus:'error', msg : "Photo unable to upload."});
...
listBrand (req, res) {
     ...
     listModels (req, res) {
          ...
          /**--------------------------------------------------------------------------
          Function    : listModelType
          Description : Use to get car model type
          --------------------------------------------------------------------------*/
          getCarModelType (req, res) {
               let brand = req.query.brand;
               let model = req.query.model;
               BrandVariantModel
                    .findOne (
                    ...
                    .exec (
                         function (err, resData) {
                              if (err) return res.json({resStatus:'error', msg : AppMessages.SERVER_ERR});
                              if(resData) {
                                   return res.json({resStatus:'success', msg :'Model Type', result : resData.type});
                              } else {
                                   return res.json({resStatus:'success', msg :'Model Type', result : 'CAR'});
                              }
          ...
          checkAvailabilty (req, res) {
               ...
               //console.log(vehicleId + "===========" +reqStartDate + "=======" + reqEndDate);
               let query = {bookingStatus : 'COMPLETED', vehicleId : vehicleId, "startDate": {"$lt": new Date(reqEndDate) }, "endDate": {"$gt": new Date(reqStartDate)}};
